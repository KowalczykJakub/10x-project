...
test.describe("Study Session", () => {
  ...
  test("user can complete study session", async ({ page }) => {
    ...
    const hasFlashcard =
      (await page.locator('.flashcard, [data-testid="flashcard"]').count()) > 0 ||
      (await page.locator("text=/question|pytanie/i").count()) > 0;
    ...
    if ((await ratingButtons.count()) > 0) {
      ...
      for (let i = 0; i < 1; i++) {
        ...
        const rating = page.locator('button:has-text("Easy"), button:has-text("Łatwe")');
      ...
      // This is OK for the test - we just verify the rating mechanism works
    }
  });

  test.skip("shows empty state when no flashcards", async () => {
  ...
  test("flashcard flips to show answer", async ({ page }) => {
    ...
    if ((await flashcardElement.count()) > 0) {
      ...
      const hasRatingButtons =
        (await page.locator('button:has-text("Hard"), button:has-text("Medium"), button:has-text("Easy")').count()) > 0;
  ...
  test("rating buttons work correctly", async ({ page }) => {
    ...
    const easyButton = page.locator('button:has-text("Easy"), button:has-text("Łatwe")');
    if ((await easyButton.count()) > 0) {
      ...
      const hasNextCard = (await page.locator('.flashcard, [data-testid="flashcard"]').count()) > 0;
      const hasCompletion = (await page.locator("text=/ukończona|completed/i").count()) > 0;
  ...
  test("progress indicator updates during session", async ({ page }) => {
    ...
    const ratingButton = page
      .locator('button:has-text("Easy"), button:has-text("Medium"), button:has-text("Hard")')
      .first();
    if ((await ratingButton.count()) > 0) {
      ...
      const newProgressText = await page.locator("text=/\\d+.*\\d+/").first().textContent();
