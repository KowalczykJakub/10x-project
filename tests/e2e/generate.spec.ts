...
test.describe("Flashcard Generation", () => {
  ...
  test.skip("user can generate flashcards from text", async () => {
  ...
  test.skip("character counter updates as user types", async () => {
    // Skipped - character counter visibility depends on implementation
  });

  test.skip("user can select and save proposals", async () => {
    // Skipped - requires AI generation test, too slow
    test.setTimeout(60000);
  });

  test.skip("user can deselect proposals before saving", async () => {
  ...
  test("user can navigate to generations history", async ({ page }) => {
    ...
    const historyLink = page.locator('a:has-text("Historia"), a:has-text("History"), button:has-text("Historia")');
