...
test.describe('Flashcard Management', () => {
  ...
  test('user can delete flashcard with confirmation', async ({ page }) => {
    ...
    if (deleteButtonCount > 0) {
      ...
      const cancelButton = page.locator(
        '[role="alertdialog"] button:has-text("Anuluj"), [role="alertdialog"] button:has-text("Cancel")'
      ).first();
  ...
  test('user can search flashcards', async ({ page }) => {
    ...
    if ((await searchInput.count()) > 0) {
      ...
      expect(
        await page.locator('text=Searchable Question').count()
      ).toBeGreaterThan(0);
    }
  });

  test.skip('empty state is shown when no flashcards exist', async ({ page }) => {
