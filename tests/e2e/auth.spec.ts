...
test.describe('Authentication Flow', () => {
  test('user can login with valid credentials', async ({ page }) => {
    ...
    const loggedIn =
      (await page.locator('text=/wyloguj/i').count()) > 0 ||
      (await page.locator('text=/profil/i').count()) > 0;
  ...
  test('registration shows validation errors for weak password', async ({
    ...
    const hasValidationError =
      (await page
        .locator('text=/minimum|musi|zawiera|znaki|znak/i')
        .count()) > 0 ||
  ...
  test('registration shows error for mismatched passwords', async ({
    page,
  }) => {
    // Arrange
    await page.goto('/register', { waitUntil: 'networkidle' });
  ...
  test('authenticated user cannot access login page (redirects)', async ({
    page,
  }) => {
    // Arrange - Login first
    await loginAsTestUser(page);
